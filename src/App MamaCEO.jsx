        ...
        <div className="bpcard">
          ...
          {editingBrand && (
            <form className="bpcard-form" onSubmit={(e) => { e.preventDefault(); setBrandProfile({ ...brandForm }); setEditingBrand(false); }}>
              <div className="bpcard-fields">
                <div className="bpcard-field-group">
                ...
                <div className="bpcard-field-group">
                  ...
                  <textarea className="bpcard-textarea" placeholder="Ej: Mamás de 28 a 40 años que quieren crecer sin descuidar a su familia" value={brandForm.clienteIdeal} onChange={(e) => setBrandForm((p) => ({ ...p, clienteIdeal: e.target.value }))} rows={2} />
                </div>
                <div className="bpcard-field-group">
                  <label className="bpcard-label">¿Cuál es la transformación clave? <span className="bpcard-req">*</span></label>
                  <textarea className="bpcard-textarea" placeholder="Ej: Pasar de agotada y desbordada a organizada, rentable y presente" value={brandForm.transformacion} onChange={(e) => setBrandForm((p) => ({ ...p, transformacion: e.target.value }))} rows={2} required />
                </div>
                <div className="bpcard-field-group">
                  <label className="bpcard-label">Tono de comunicación</label>
                  <div className="bpcard-pills">
                    {["Cercano", "Profesional", "Inspirador", "Directo"].map(t => (
                      <button key={t} type="button" className={`bpcard-pill${brandForm.tono === t ? " active" : ""}`} onClick={() => setBrandForm(p => ({ ...p, tono: t }))}>{t}</button>
                    ))}
                  </div>
                </div>
                <div className="bpcard-field-group">
                  ...
                  <div className="bpcard-pills">
                    {["Instagram", "TikTok", "YouTube", "Spotify"].map(r => (
                      <button key={r} type="button" className={`bpcard-pill${brandForm.redPrincipal === r ? " active" : ""}`} onClick={() => setBrandForm(p => ({ ...p, redPrincipal: r }))}>{r}</button>
                    ))}
                  </div>
                </div>
                <div className="bpcard-field-group">
                  <label className="bpcard-label">Hashtags favoritos <span className="bpcard-opt">(opcional)</span></label>
                  <input className="bpcard-input" placeholder="#mamaceo #emprendedora #maternidadconsciente" value={brandForm.hashtags} onChange={(e) => setBrandForm((p) => ({ ...p, hashtags: e.target.value }))} />
                </div>
              </div>
              <div className="bpcard-form-footer">
                <button className="brand-save-btn" type="submit">Guardar perfil de marca</button>
